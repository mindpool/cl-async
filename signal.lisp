  ...
  (when (find signo *signal-handlers*)
    (let* ((signo-sym (signal-sym signo))
      ...
      (cffi:foreign-funcall "signal"
                            :int signo
                            :pointer original-lisp-signal-handler
                            :pointer)
...
(defun set-lisp-signal-handler (signo fn)
  ...
  (cffi:foreign-funcall "signal" :int signo :pointer
    (cffi:callback lisp-signal-cb) :pointer))

(cffi:defcallback signal-cb :void 
  ((signo :int) (event :short) (data-pointer :pointer))
...
(defun signal-handler (signo signal-cb &key event-cb)
  "Setup a one-time signal handler for the given signo. This also sets up a
   ...
   instead of the event loop, it will run the same callback. All signal
   handlers are restored on event loop exit."
  ...
  (let* ((data-pointer (create-data-pointer))
         (ev (le:event-new *event-base* signo 
           (logior le:+ev-signal+ le:+ev-persist+)
           (cffi:callback signal-cb) data-pointer))
         (lisp-signal-handler
           (set-lisp-signal-handler signo (lambda () (le:event-active ev))))
         (signo-sym (signal-sym signo)))
    (le:event-add ev (cffi:null-pointer))
    (save-callbacks data-pointer 
      (list :signal-cb signal-cb :event-cb event-cb))
    (attach-data-to-pointer data-pointer
      (list :ev ev :original-handler lisp-signal-handler))
